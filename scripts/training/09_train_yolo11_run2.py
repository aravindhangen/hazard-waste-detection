...
RUN2_ROOT = RUN2_DIR
RUN2_TRAIN_DIR = RUN2_ROOT / "train"
RUN2_WEIGHTS_DIR = RUN2_ROOT / "weights"
RUN2_EVAL_DIR = RUN2_ROOT / "evaluation"
...
def ensure_run2_layout() -> None:
    RUN2_WEIGHTS_DIR.mkdir(parents=True, exist_ok=True)
    RUN2_EVAL_DIR.mkdir(parents=True, exist_ok=True)
...
def train_run2(
    ...
    train_kwargs = {
        ...
        "device": device,
        "workers": workers,
        "project": str(RUN2_ROOT),
        "name": "train",
        "exist_ok": True,
        "pretrained": True,
        "verbose": True,
    }
    last_ckpt = RUN2_TRAIN_DIR / "weights" / "last.pt"
    if resume and last_ckpt.exists():
        train_kwargs["resume"] = str(last_ckpt)
        print(f"Resuming from checkpoint: {last_ckpt}")

    print("\nStarting Run 2 training (YOLO11s-Seg)...")
    print(f"  Output directory: {RUN2_ROOT}")
    model.train(**train_kwargs)

    source_best = RUN2_TRAIN_DIR / "weights" / "best.pt"
    if not source_best.exists():
        raise FileNotFoundError(f"Run 2 best weights not found: {source_best}")

    target_best = RUN2_WEIGHTS_DIR / BEST_RUN2_NAME
    shutil.copy2(source_best, target_best)
    print(f"\nCopied Run 2 checkpoint to: {target_best}")
    return target_best


def extract_metrics(results, split: str, class_names: list[str]) -> SplitMetrics:
...
def evaluate_run2(
    ...
    print("\nRunning Run 2 validation evaluation...")
    val_results = model.val(
        ...
        batch=batch_size,
        device=device,
        project=str(RUN2_EVAL_DIR),
        name="val",
        exist_ok=True,
        verbose=True,
    )
    val_metrics = extract_metrics(val_results, "val", list(CLASS_NAMES.values()))

    print("\nRunning Run 2 untouched test evaluation...")
    ...
    test_metrics = extract_metrics(test_results, "test", list(CLASS_NAMES.values()))

    print("\nBenchmarking Run 2 inference speed (batch=1)...")
...
def write_run2_report(
    ...
    report_txt = RUN2_EVAL_DIR / "run2_evaluation_report.txt"
    report_json = RUN2_EVAL_DIR / "run2_evaluation_report.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-train", action="store_true", help="Evaluate existing Run 2 weights only.")
    parser.add_argument(
        "--weights",
        type=str,
        default="",
        help="Optional weights path for --skip-train.",
    )
    parser.add_argument("--resume", action="store_true", help="Resume Run 2 training from last.pt.")
...
def main() -> None:
    ...
    if args.skip_train:
        weights = Path(args.weights) if args.weights else RUN2_WEIGHTS_DIR / BEST_RUN2_NAME
        if not weights.exists():
            fallback = RUN2_TRAIN_DIR / "weights" / "best.pt"
            if fallback.exists():
                weights = fallback
            else:
                raise FileNotFoundError(f"Run 2 weights not found: {weights}")
    else:
        weights = train_run2(
            device=device,
            epochs=args.epochs,
    ...
    val_metrics, test_metrics, infer_ms = evaluate_run2(
        weights=weights,
        device=device,
        batch_size=args.batch_size,
        img_size=args.img_size,
    ...
    print("\n" + "=" * 70)
    print("RUN 2 EVALUATION COMPLETE")
    print("=" * 70)
    ...
    print("\nNext: python 10_compare_runs.py")
