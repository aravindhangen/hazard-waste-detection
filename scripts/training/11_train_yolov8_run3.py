...
RUN3_ROOT = RUN3_DIR
RUN3_TRAIN_DIR = RUN3_ROOT / "train"
RUN3_WEIGHTS_DIR = RUN3_ROOT / "weights"
RUN3_EVAL_DIR = RUN3_ROOT / "evaluation"
...
def ensure_run3_layout() -> None:
    RUN3_WEIGHTS_DIR.mkdir(parents=True, exist_ok=True)
    RUN3_EVAL_DIR.mkdir(parents=True, exist_ok=True)
...
def train_run3(
    ...
    train_kwargs = {
        ...
        "device": device,
        "workers": workers,
        "project": str(RUN3_ROOT),
        "name": "train",
        "exist_ok": True,
        "pretrained": True,
        "verbose": True,
    }
    last_ckpt = RUN3_TRAIN_DIR / "weights" / "last.pt"
    if resume and last_ckpt.exists():
        train_kwargs["resume"] = str(last_ckpt)
        print(f"Resuming from checkpoint: {last_ckpt}")

    print("\nStarting Run 3 training (YOLOv8s-Seg)...")
    print(f"  Output directory: {RUN3_ROOT}")
    model.train(**train_kwargs)

    source_best = RUN3_TRAIN_DIR / "weights" / "best.pt"
    if not source_best.exists():
        raise FileNotFoundError(f"Run 3 best weights not found: {source_best}")

    target_best = RUN3_WEIGHTS_DIR / BEST_RUN3_NAME
    shutil.copy2(source_best, target_best)
    print(f"\nCopied Run 3 checkpoint to: {target_best}")
    return target_best
...
def evaluate_run3(
    ...
    print("\nRunning Run 3 validation evaluation...")
    ...
    print("\nRunning Run 3 untouched test evaluation...")
    ...
    print("\nBenchmarking Run 3 inference speed (batch=1)...")
...
def write_run3_report(
    ...
    report_txt = RUN3_EVAL_DIR / "run3_evaluation_report.txt"
    report_json = RUN3_EVAL_DIR / "run3_evaluation_report.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--skip-train", action="store_true", help="Evaluate existing Run 3 weights only.")
    parser.add_argument("--weights", type=str, default="", help="Optional weights path for --skip-train.")
    parser.add_argument("--resume", action="store_true", help="Resume Run 3 training from last.pt.")
...
def main() -> None:
    ...
    if args.skip_train:
        weights = Path(args.weights) if args.weights else RUN3_WEIGHTS_DIR / BEST_RUN3_NAME
        if not weights.exists():
            fallback = RUN3_TRAIN_DIR / "weights" / "best.pt"
            if fallback.exists():
                weights = fallback
            else:
                raise FileNotFoundError(f"Run 3 weights not found: {weights}")
    else:
        weights = train_run3(
            device=device,
            epochs=args.epochs,
    ...
    val_metrics, test_metrics, infer_ms = evaluate_run3(
        weights=weights,
        device=device,
        batch_size=args.batch_size,
        img_size=args.img_size,
    ...
    print("\n" + "=" * 70)
    print("RUN 3 EVALUATION COMPLETE")
    print("=" * 70)
